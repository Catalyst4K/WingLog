...
/** Reads and parses `<folderPath>/pmdg-aircraft-77w/work/Aircraft/<registration>.hours`
 ...
 *
 *  Returns null for every reason there's nothing to show: no folder configured, the file
 *  doesn't exist (this aircraft isn't a PMDG 777, or it is but hasn't flown with this folder
 *  configured yet), or any other read error — detection is deliberately just "did the read
 *  succeed" rather than a hardcoded icaoType allow-list that would go stale
 *  (docs/plans/fleet-maintenance.md Part 1). Never throws.
 *
...
export async function readPmdg777Maintenance(
  ...
  const aircraftDir = resolve(join(folderPath, 'pmdg-aircraft-77w', 'work', 'Aircraft'))
